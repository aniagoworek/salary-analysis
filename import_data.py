...
def transform_data(df):
    ...
    df_2016_2017 = df.filter(regex='- 201[67]') #0-4, 5-9, 10-14, 15-19, 20-24, 25-29
...
def split_data(df_2016_2017, df_2016_2018, df_2016_2019, df_2016_2020, df_2016_2021, df_2016_2022):
    ...
    # print(f'\nDane poczatkowe X: {testing_sets[2][0]}') # pierwsza liczba oznacza numer zbioru, druga: 0 - X, 1 - y
    # print(f'\nDane poczatkowe y: {training_sets[2][1]}') # pierwsza liczba: 0-29 = lata 2016-2017, 30-59 = lata 2016-2018, 60-89 = lata 2016-2019, 90-119 = lata 2016-2020, 120-149 = lata 2016-2021, 150-179 = lata 2016-2022
...
def split_data_without_salary(df_2016_2017, df_2016_2018, df_2016_2019, df_2016_2020, df_2016_2021, df_2016_2022):
    ...
    for df_year in [df_2016_2017, df_2016_2018, df_2016_2019, df_2016_2020, df_2016_2021, df_2016_2022]:
        X = df_year.drop(columns=df_year.filter(regex='wynagrodzenia').columns)
        y = df_year[f'wynagrodzenia - {df_year.columns[-1][-4:]}']
